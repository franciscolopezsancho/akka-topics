...
import akka.persistence.typed.scaladsl.{ ReplyEffect, Effect, EventSourcedBehavior }
...
object Wallet {
	 ...
	 case class ReserveFunds(amount: Int, replyTo: ActorRef[UpdatedResponse]) extends Command //have the id on the ActorRef?
	 case class AddFunds(amount: Int, replyTo: ActorRef[UpdatedResponse]) extends Command //have the id on the ActorRef?
		...
		command match {
			case ReserveFunds(amount, replyTo) =>
				...
				if (amount > state.balance) 
					Effect.persist(FundsReserved(amount)).thenReply(replyTo)(state => Accepted)
				else 
					Effect.persist(FundsReservationDenied(amount)).thenReply(replyTo)(state => Rejected)
			case AddFunds(amount, replyTo) => Effect.persist(FundsAdded(amount)).thenReply(replyTo)(state => Accepted)
			case CheckFunds(replyTo) => Effect.reply(replyTo)(CurrentBalance(state.balance))
		}
	}
