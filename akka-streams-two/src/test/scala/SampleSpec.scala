...
// https://jessitron.com/2014/01/29/choosing-an-executorservice/
  ...
  "connecting to a db" should {
    "work" ignore {
      ...
      def getPersistenceId(id: Int): Option[String] = {
      ...
      //find throttle to send 1000 queries
      val result = Source(1 to 100)
        .mapAsync(100)(each => Future(getPersistenceId(each % 10)))
        .map(println)
        .run()
  ...
  "an Exception" should {
    "be possible to overcome by continuing" ignore {
